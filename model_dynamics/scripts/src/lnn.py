...
def accelerationFull(n, Dim, lagrangian=lagrangian, non_conservative_forces=None, external_force=None,
                     constraints=None):
    ...
    if non_conservative_forces == None:
        def non_conservative_forces(x, v, x_lead, v_lead, params): return 0
    if external_force == None:
        def external_force(x, v, x_lead, v_lead, params): return 0
    if constraints == None:
    ...
    def fn(x, v, x_lead, v_lead, params):
        N = n * Dim
        # M⁻¹ = (∂²L/∂²v)⁻¹
        M = d2L_dv2(x.flatten(), v.flatten(), params)
        M_1 = inv(M)
        # Π = ∂L/∂x
        Π = jax.grad(lagrangian, 0)(x, v, params).reshape(N, 1)
        # C = ∂²L/∂v∂x
        C = jax.jacobian(jax.jacobian(lagrangian, 1),
                         0)(x, v, params).reshape(N, N)
        Υ = non_conservative_forces(x, v, x_lead, v_lead, params)
        F = external_force(x, v, x_lead, v_lead, params)
...
def useNN(norm=True):
    ...
    if norm:
        def f(x, params=None, cutoff=None):
            ...
            return jnp.where(x_ < cutoff, forward_pass(params, x_, activation_fn=SquarePlus),
                             forward_pass(params, cutoff, activation_fn=SquarePlus))
...
def SPRING_general(x, stiffness=1.0, length=1.0):
    """Linear spring, v=0.5kd^2.
    ...
    :rtype: float
    """
    return 0.5 * stiffness * (x - length) ** 2
...
def angular_mom_fn(states):
    return jnp.array(
        [jnp.sqrt(jnp.square(jnp.cross(state.position, state.velocity).sum(axis=0)).sum()) for state in states])
